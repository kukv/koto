...
MCP サーバが起動しないときは、まず `DATABASE_URL` の形式を疑ってください(`postgres://ユーザー:パスワード@ホスト:ポート/DB名` の形式でないと起動時に弾かれます)。Claude Code の MCP ログで詳しいエラーを確認できます。
...
書き込んだ内容だけを見ます(ファイル全体は読みません)。DB に繋がらないときは何もしません — 知識基盤の警告で編集作業が止まらないようにするためです。1 文字の禁止表記は誤検知が多いため対象外です。ただし `DATABASE_URL` の**形式**が不正なとき(接続失敗とは別)は、この経路でもフックがエラー終了します。
...
承認・却下・検証レベルの設定は MCP ツール(`approve_knowledge` / `reject_knowledge` / `verify_knowledge`)から行いますが、**実行するとサーバが確認ダイアログを出し、ユーザー自身が確認者を選んで承認するまで DB は変更されません**。エージェントが勝手に承認することはできません。確認者の候補は環境変数 `KOTO_REVIEWER` で設定します(未設定だと実行できません)。承認・検証では根拠(何を読んで裏を取ったか)の記載が必須で、確認ダイアログに表示されたうえで記録に残ります。承認時には、出典の文書から意図して落とした記述もこの根拠に書いておきます。同じ文書をもう一度取り込んだときに、その記述が再び提案されるのを防ぐためです。確認ダイアログには、その領域の承認責任者(`contexts.owner`)が表示され、設定されていない場合は警告が出ます。誰がその知識の正しさを判定できるかが記録されていない状態なので、`upsert_context` で owner を設定してください。
