...
export function createKotoServer(): McpServer {
  ...
  server.registerTool(
    "approve_knowledge",
    {
      title: "知識の承認",
      description:
        "レビュー待ちの知識を承認し、検索の既定対象にする。承認すると未解決の確認事項(review_notes)はクリアされ、代わりに note が承認根拠として残る。note には何を読んで裏を取ったかを具体的に書くこと(この記録自体が後から参照される知識になる)。出典の文書から意図して落とした記述があれば、それも note に書くこと(同じ文書をもう一度取り込んだときに、その記述が再び提案されるのを防ぐ)。同じ english_name を持つ既存レコードがあれば確認ダイアログに表示される — 統合すべきか別概念かはユーザーが判断する。実行するとユーザーに確認ダイアログが出る。ユーザーが承認しなければ何も変更されない。承認するかどうかの判断は必ずユーザーに委ねること。",
      inputSchema: {
        id: z.string().uuid(),
        note: z
          .string()
